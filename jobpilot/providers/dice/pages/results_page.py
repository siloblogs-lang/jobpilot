...
class ResultsPage(BasePage):
    ...
    def _extract_card_payload(self, idx: int):
        cards = self._card()
        if idx >= len(cards):
            return None
    ...
    def iterate_all(self, max_results: int = 100) -> List[JobPosting]:
        ...
        while len(jobs) < max_results:
            ...
            for idx in range(current_card_count):
            # for idx, card in enumerate(cards):
                # url = self._safe_href(card, RESULT_LINK)
                # if not url:
                #     continue
                try:
                    payload = self._extract_card_payload(idx)
                except StaleElementReferenceException:
                    print(f"[ResultsPage] stale card at idx={idx}, retrying once")
                    continue

                if not payload:
                    continue

                job_id = self._make_job_id(payload["url"])
                ...
                jobs.append(
                    JobPosting(
                        ...
                        metadata={
                            "source_card_index": str(idx),
                            "raw_company_url": payload["raw_company_url"]
                        },
                        # title=title,
                        # company=company,
                        # location=location,
